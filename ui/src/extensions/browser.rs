...
pub mod browser {
    use common::{state::State, icons::outline::Shape as Icon, language::get_local_text, STATIC_ARGS};
    ...
    pub fn ExtensionsBrowser(cx: Scope) -> Element {
        ...
        cx.render(rsx!(
            div {
                ...
                (active_route.get().to_owned() == "installed").then(|| rsx!(
                    Installed {}
                )),
                (active_route.get().to_owned() == "explore").then(|| rsx!(
                    Explore {}
                )),
                (active_route.get().to_owned() == "settings").then(|| rsx!(
                    Settings {}
                ))
            }
        ))
